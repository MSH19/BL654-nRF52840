...
/*
  Application configuration structure.
    Specified by user from template.
...
AppAMPCfg_Type AppAMPCfg =
{
  ...
  .SensorBias = 300,                          /* Sets voltage between RE0 and SE0 */
...
{
    fImpPol_Type RtiaCalValue;  /* Calibration result (Pol type) */
...
AD5940Err AppAMPInit(uint32_t *pBuffer, uint32_t BufferSize)
{
    ...
    /*
    // Do RTIA calibration
    if(((AppAMPCfg.ReDoRtiaCal == bTRUE) || AppAMPCfg.AMPInited == bFALSE) && AppAMPCfg.ExtRtia == bFALSE)
    {
        AppAMPRtiaCal();
...
AD5940Err AppAMPISR(void *pBuff, uint32_t *pCount)
{
  ...
  *pCount = 0;
  ...
  if(AD5940_INTCTestFlag(AFEINTC_0, AFEINTSRC_DATAFIFOTHRESH) == bTRUE)
  {
      ...
      // char sendBuf[4];
      // sendBuf[3] = (char) FifoCnt & 255;
      // sendBuf[2] = (char) (FifoCnt  >> 8) & 255;
      // sendBuf[1] = (char) (FifoCnt  >> 16) & 255;
      // sendBuf[0] = (char) (FifoCnt  >> 24) & 255;
